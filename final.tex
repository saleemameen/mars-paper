...
\Authors{Alex Sharp\textsuperscript{1}*, Saleem Ameen\textsuperscript{2}, John Walker\textsuperscript{3}, Epi Pereria\textsuperscript{4}} % Authors
\affiliation{\textsuperscript{1}\textit{OrionVM, University of Sydney Alumni}} % Author 1 affiliation

%TODO Is this ok?


\affiliation{\textsuperscript{2}\textit{??}} % Author 2 affiliation
\affiliation{\textsuperscript{3}\textit{??}} % Author 3 affiliation
\affiliation{\textsuperscript{4}\textit{Artist @id\textunderscore epifanio}} % Author 2 affiliation
...
%%% TODO: \subsection{The Structure of the Bulk Materials Market} is new and needs a go over. I think the tie in between the economic section and the production section is going to make us unique overall, as I haven't seen another group do that before.
%%% TODO \subsection{Architecture} is also new and needs a go over.
%%% TODO \subsection{Critical services} is also new and needs a go over.
%---------------------------------------------------------------------------------
...
% For example, \ce{CO_2} separated through amine or ionic liquid scrubbing \cite{Lotto2018,StephenYates2016} is used a chemical input
...
%TODO Where would we put things like Penecillin, which are valuable, but aren't really produced in bulk.
...
\end{tabular}
\caption{Table of bulk products} %TODO is this bulk products or materials?
\label{tab:bulk_products}
\end{table*}

% \subsection{Other materials } ?
% Include penicillin here?

\subsubsection{Raw Materials and the Mining Process}
\label{sec:necessities_raw}

There are a number of raw materials, such as iron and steel, that are required to build and sustain the economy of KCSAR, which can be acquired through mining. Mining bases on Mars need to be small and relatively self-sufficient, since transporting oxygen and water is not economically viable. As such, mining operations use a ZSI oxygen generator, powered by a mass-produced kilopower-style \ref{KilopowerNASA} reactor. Water and \ce{CO_2} is extracted from the air using ionic liquids, which become inputs to the ZSI plant and the water recycling plant. The water recycling plant takes in recycled waste water, extracted via the TSSE process detailed in section \ref{sec:necessities_water}, and rejects concentrated brine as waste. This process is sufficiently efficient to allow for metabolic water production and imported foods to meet the water needs of the small base.
...
%TODO Should we shove the penecillin, etc. here?
...
%TODO THIS IS NEW IT MUST BE CHECKED
...
\item Mars is an ideal candidate for large scale nuclear production, so isotropic extraction from spent fuel will be a significant source of revenue. Nuclear isotopes such as \ce{^240Pu}, \ce{^90Sr}, \ce{^241Am} and \ce{^60Co} are extremely expensive on Earth but would be relatively easy to produce on Mars. \ce{^240Pu} is a major component in radiothermal generators, used in deep space probes that are likely to be built nearby. Similarly, extraction of fission product rare earth elements provides a non-terrestrial source of these indispensable elements.
