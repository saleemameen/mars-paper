...
Thorium breeder shell on top of the fast neutron reactor

Cryogenic energy storage via LN2 - heated via industrial waste heat (to boil LN2 to N2). N2 used to pressurize tanks. N2 feed taken to the reactor to be heated, passed through turbines, an isovolumetric heat exchanger (to regenerate heat) before being discharged to atmosphere.

LN2 used to maintain LO2 tanks, as LN2 boils above LO2, so having two tanks in thermal contact will keep the LO2 cool and low pressure.
...
Created the precedent that 'Peaceful and continuous display of territorial sovereignty is as good as title'. Sitting within the framework of Terra Nullius for the initial creation of a claim, this provides a basis for an ongoing claim.
...
Quantum dot greenhouses?

Starship rocket costs - primarily reduced by increasing starship lifespan, at least to the 10 year mark. Then cost drops in other factors help. The major component is that the market for rocket travel needs to be reasonably competitive otherwise prices don't match costs - rocket production companies and launch providers should not be vertically integrated.
...
TODO Nutritional information - Vitamin C/D/Iodine deficiency is likely going to be widespread, so foods will need to be fortified with artificial sources.
...
TODO Food - Some spices are easy to engineer either chemically (ie. simple esters) or biologically (ie. limonine, linalool). These flavours would then be orders of magnitude cheaper than flavors produced via hydroponic spices, which would then be orders of magnitude cheaper than needing imported spices from earth. This will change the palate of Martians, as the food they're accustomed to eat will be influenced by this.
...
TODO:
- Futures market for industrial products to provide stability for demand/price, allowing for banks to lend for large scale capital expenditure.
- Alkane cracking to methane -> methane reforming to form hydrogen.
- Ammonia decomposition via Na/NaNH2 to also form hydrogen.
- Multiple overlapping supply curves with different characteristics. ie. electrolysis units have low fixed cost but high marginal/average cost, farms are a massive capital project but have very low average cost, S/I reactors have medium capital costs, and medium average cost of production.
- Multiple overlapping supply curves induce elasticity in supply + demand. Inelastic supply/demand curves form an unstable equilibrium, which can be fatal. Form a series of artificial 'automatic stabilisers' to help ensure a stable equilibrium.
...
r polustrus -> Most versitile thing https://en.wikipedia.org/wiki/Rhodopseudomonas_palustris
...
Farming comes in many forms and at many price points. From eatable industrial inputs (such as bulk glucose or protein powder), through to battery chickens and fish. 
...
Genetic engineering of humans is something that is either technically or ethically out of the grasp of humanity, and is likely to be for some time. There are other techniques that allow for similar outcomes while being significantly less powerful, however. Embrionic selection is one of these techniques.
